...
import com.shyrack.flansmodupgraded.client.handlers.FlansModResourceHandler;
import com.shyrack.flansmodupgraded.common.FlansMod;
import com.shyrack.flansmodupgraded.common.PlayerHandler;
import com.shyrack.flansmodupgraded.common.network.PacketMGFire;
import com.shyrack.flansmodupgraded.common.network.PacketMGMount;
import com.shyrack.flansmodupgraded.common.network.PacketPlaySound;
import com.shyrack.flansmodupgraded.common.teams.EntityGunItem;
...
public class EntityMG extends Entity implements IEntityAdditionalSpawnData {
    ...
    public EntityMG(Level level) {
        super(level);
        setSize(1.0F, 1.0F);
        ignoreFrustumCheck = true;
    }

    public EntityMG(Level level, int x, int y, int z, int dir, GunType gunType) {
        super(level);
        setSize(1.0F, 1.0F);
        blockX = x;
        blockY = y;
    ...
    public void onUpdate() {
        ...
        if (level.getBlockState(new BlockPos(blockX, blockY - 1, blockZ)).getBlock() == Blocks.AIR) {
            if (!level.isRemote) {
                setDead();
            }
        }
        ...
        if (level.isRemote && gunner != null && gunner == FMLClientHandler.instance().getClient().player && type.mode == EnumFireMode.FULLAUTO) {
            //Send a packet!
            checkForShooting();
        }
        if (!level.isRemote && isShooting) {
            fire();
        }
        if (soundDelay > 0)
            soundDelay--;
    ...
    private void fire() {
        ...
        ShotHandler.fireGun(level, shot, type.numBullets * shootableType.numBullets, position, shootingDirection, handler);
    ...
    public void mountGun(Player player, boolean mounting) {
        if (player == null)
            return;
        Side side = level.isRemote ? Side.CLIENT : Side.SERVER;
    ...
    public void setDead() {
        ...
        if (!level.isRemote) {
            if (TeamsManager.weaponDrops == 2) {
                EntityGunItem gunEntity = new EntityGunItem(level, posX, posY, posZ, new ItemStack(type.getItem()), Arrays.asList(ammo));
                level.spawnEntity(gunEntity);
