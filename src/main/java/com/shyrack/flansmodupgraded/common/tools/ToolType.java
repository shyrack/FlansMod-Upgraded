package com.shyrack.flansmodupgraded.common.tools;
...
public class ToolType extends InfoType {
    ...
    protected void read(String[] split, TypeFile file) {
        ...
        try {
            if (FMLCommonHandler.instance().getSide().isClient() && split[0].equals("Model"))
                model = FlansMod.proxy.loadModel(split[1], shortName, Model.class);
    ...
    public void addRecipe(IForgeRegistry<IRecipe> registry, Item item) {
        ...
        if (rechargeRecipe.size() < 1)
            return;
        rechargeRecipe.add(Ingredient.of(new ItemStack(item, 1, toolLife)));

        NonNullList<Ingredient> ingredients = NonNullList.create();
        ingredients.addAll(rechargeRecipe);
        registry.register(new ShapelessRecipe("FlansMod", new ItemStack(item, 1, 0), ingredients).setRegistryName(name + "_recharge"));
    }
